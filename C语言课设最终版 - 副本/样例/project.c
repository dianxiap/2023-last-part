...
{
    return ((cou*)b)->people - ((cou*)a)->people;
}

int cmp_courseID(const void* a, const void* b)
{
    return strcmp(((cou*)a)->course_ID, ((cou*)b)->course_ID);
}

int cmp_credit(const void* a, const void* b)
{
    float delta = atof(((cou*)b)->credit) - atof(((cou*)a)->credit);
    if (delta > 0)
        return 1;
    else if (delta < 0)
        return -1;
...
void show_cou(Stu* spc, Cou* cpc, Sel* lpc) {
    ...
    case 2:
        ...
        break;
    case 3:
        qsort(cpc->cou, cpc->size + 1, sizeof(cou), cmp_people);
        show_cou(spc, cpc, lpc);
        break;
    case 4:
        qsort(cpc->cou, cpc->size + 1, sizeof(cou), cmp_courseID);
        show_cou(spc, cpc, lpc);
        break;
    case 5:
        qsort(cpc->cou, cpc->size + 1, sizeof(cou), cmp_credit);
        show_cou(spc, cpc, lpc);
        break;
    default:
        system("cls");
