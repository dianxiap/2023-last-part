...
int cmp_stu_id(const void* a, const void* b) {
    stu* pa = (stu*)a;
    stu* pb = (stu*)b;
    return strcmp(pa->stu_ID, pb->stu_ID);
}

int cmp_stu_name(const void* a, const void* b) {
    stu* pa = (stu*)a;
    stu* pb = (stu*)b;
    return strcmp(pa->stu_name, pb->stu_name);
}

int cmp_stu_grade(const void* a, const void* b) {
    stu* pa = (stu*)a;
    stu* pb = (stu*)b;
    return strcmp(pa->grade, pb->grade);
}
...
void show_stu(Stu* spc, Cou* cpc, Sel* lpc) {
    ...
    case 2:
        ...
        break;
    case 3: // 按照学号排序
        qsort(spc->st, spc->size + 1, sizeof(stu), cmp_stu_id);
        show_stu(spc, cpc, lpc);
        break;
    case 4: // 按照姓名排序
        qsort(spc->st, spc->size + 1, sizeof(stu), cmp_stu_name);
        show_stu(spc, cpc, lpc);
        break;
    case 5: // 按照班级排序
        qsort(spc->st, spc->size + 1, sizeof(stu), cmp_stu_grade);
        show_stu(spc, cpc, lpc);
        break;
    default:
        system("cls");
